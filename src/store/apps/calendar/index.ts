...
export const deleteEvent = createAsyncThunk('appCalendar/deleteEvent', async (id: number | string, { dispatch }) => {
  ...
  // await dispatch(fetchEvents(['full_paid', 'Business', 'Family', 'Holiday', 'ETC']))
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: null,
    selectedCalendars: ['full_paid', 'partially_paid']
  },
  reducers: {
    ...
    handleAllCalendars: (state, action) => {
      ...
      if (value === true) {
        state.selectedCalendars = ['full_paid', 'partially_paid']
      } else {
