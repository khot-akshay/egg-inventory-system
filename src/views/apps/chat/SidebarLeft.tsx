...
import BulkMsgDialog from './BulkMessage'
...
const SidebarLeft = (props: ChatSidebarLeftType) => {
  ...
  const [openBulkMsg, setOpenBulkMsg] = useState(false)
  const [inputVal, setInputVal] = useState("");

  // ** Hooks
  const router = useRouter()

  const handleChatClick = (type: 'chat', id) => {
    // dispatch(selectChat(id))
    selectChat(id)
    setActive({ type: 'chat', id: id.id })
    if (!mdAbove) {
      handleLeftSidebarToggle()
    }
  ...
  const renderChats = () => {
    if (store && store.chats && store.chats.length) {
      if (!operators.length) {
        return (
          <ListItem>
            <Typography sx={{ color: 'text.secondary' }}>No Operator Found</Typography>
          </ListItem>
        )
      } else {
        // const arrToMap = query.length && filteredChat.length ? filteredChat : store.chats

        return operators?.filter((filteredValue) => filteredValue?.name && filteredValue.name.toLowerCase().includes(inputVal?.toLowerCase()))
        .map((chat: ChatsArrType, index: number) => {
              ...
              >
                <ListItemAvatar sx={{ m: 0 }}>
                  ...
                  >
                    {chat?.name ? (
                      <MuiAvatar
                        src={generateRandomAvatarUrl(chat.name)}
                        alt={chat.name}
                        sx={{
                          width: 38,
                          height: 38,
                ...
                <ListItemText
                  ...
                  secondary={
                    <Typography noWrap variant='body2' sx={{ ...(!activeCondition && { color: 'text.disabled' }) }}>
                      {chat?.last_telegram_message ? chat?.last_telegram_message : null}
                    </Typography>
                  }
                />
                {/* <Box
  ...
  return (
    <div>
      ...
      >
        ...
        >
         <Button onClick={()=>setOpenBulkMsg(true)} variant='contained'>Broadcast Message</Button>
      ...
      /> */}
              {openBulkMsg ? <BulkMsgDialog show={openBulkMsg} handleclose={() =>setOpenBulkMsg(false)} store={operators} isTemplateMsg={false} /> : null}
