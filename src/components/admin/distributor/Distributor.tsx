...
import AddProducts from './AddDistributor';
import { useRouter } from 'next/router';
import RHFAutoComplete from 'src/hook-forms/RHFAutoComplete';
import AddExpense from './AddDistributor'
...
const Distributor = () => {
  ...
  const fetchGame = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/api/v1/admin/getAllDistributorRoutes?${params.toString()}`
      )
  ...
  return (
    <>
      ...
      <Card sx={{ p: 5 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
          ...
          <Box display="flex" alignItems="center" gap={2}>
            <GoBack label="Distributor" isBack={false} />
          </Box>
          ...
          <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
            ...
            <Grid item xs={12} sm="auto">
              ...
              <Button onClick={() => setOpenAdd(true)} variant='contained'>
                Add Distributor <AddCircleOutlineIcon sx={{ ml: 1 }} />
              </Button>
      ...
      {openAdd && <AddDistributor open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
      {openDelete && (
      ...
      {openEdit && (
        <AddDistributor open={openEdit} handleClose={() => setOpenEdit(false)}
          fetchData={fetchGame}
