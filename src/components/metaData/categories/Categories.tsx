...
const Categories = () => {
  ...
  const fetchGame = async () => {
    setLoading(true);
    try {
      let url = `/api/v1/admin/getAllCategories?pageNo=${page}&limit=${pageSize}`;
      ...
      setRows((response.data.data?.categories ?? []) as CategoryRow[]);
      setTotalRows(response.data.data?.count ?? 0);
    } catch (e) {
      } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <Card sx={{ p: 5 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
          ...
          <Box display="flex" alignItems="center" gap={2}>
            <GoBack label="Categories" isBack={false} />
          </Box>
      ...
      {openAdd && <AddCategories open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
      {openDelete && (
        <DeleteDialogPopup show={openDelete} handleclose={() => setOpenDelete(false)} selectedItems={selectedItem?.id}
          fetchData={fetchGame}
          label={'Are you sure! You want to delete.'} apiUrl={'api/v1/admin/categories/deleteCategories/'} />
      )}
      {openEdit && (
        <AddCategories open={openEdit} handleClose={() => setOpenEdit(false)}
          fetchData={fetchGame}
