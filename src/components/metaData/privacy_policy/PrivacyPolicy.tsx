...
const PrivacyPolicy = () => {
    ...
    const fetchGame = async () => {
        setLoading(true)
        let url = `/api/v1/admin/getAllFaqs?type=privacy_policy&pageNo=${page}&limit=${pageSize}`;
  ...
  {params.row?.model_type
    ...
    : "NA"}
        ...
        },
        {
            ...
            renderCell: (params: GridCellParams) => (
                <>
                    {/* {checkPermission('update_privacy_and_policy') && ( */}

                        <Tooltip title='Update Privacy & Policy.' placement='bottom'>
                    ...
                    {/* {checkPermission('delete_privacy_and_policy') && ( */}

                        <Tooltip title='Delete Privacy & Policy.' placement='bottom'>
                            <Button
    ...
    return (
        <>
            <Card sx={{ p: 5 }} className="bg-gray-50">
                <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
                    ...
                    <Box display="flex" alignItems="center" gap={2}>
                        <GoBack label={'Privacy Policy'} isBack={false} />
                    </Box>
                    ...
                    <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
                        ...
                        {/* {checkPermission('add_privacy_and_policy') && ( */}
                            <Grid item xs={12} sm="auto">
                                <Button onClick={() => setOpenAdd(true)} variant='contained'>
                                    Add Privacy Policy <AddCircleOutlineIcon sx={{ ml: 1 }} />
                                </Button>
                            </Grid>
                            {/* )} */}
                    </Box>
            ...
            {openAdd && <AddPrivacyPolicy open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
            {openDelete && (
                <DeleteDialogPopup show={openDelete} handleclose={() => setOpenDelete(false)} selectedItems={selectedItem.id}
                    fetchData={fetchGame}
                    label={'Are you sure! You want to delete.'} apiUrl={'api/v1/admin/deleteFaq?type=privacy_policy&id='} />
            )}
            {openEdit && (
                <AddPrivacyPolicy open={openEdit} handleClose={() => setOpenEdit(false)}
                    fetchData={fetchGame}
