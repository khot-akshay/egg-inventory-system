...
import AddTermsConditions from './AddTermsConditions';
...
const TermsConditions = () => {
    ...
    const fetchGame = async () => {
        setLoading(true)
        let url = `/api/v1/admin/getAllFaqs?type=terms_and_conditions&pageNo=${page}&limit=${pageSize}`;
  ...
  {params.row?.model_type
    ...
    : "NA"}
        ...
        },
        {
            ...
            renderCell: (params: GridCellParams) => (
                <>
                    {/* {checkPermission('update_terms_and_conditions') && ( */}

                        <Tooltip title='Update Terms & Conditions.' placement='bottom'>
                    ...
                    {/* {checkPermission('update_terms_and_conditions') && ( */}
                        ...
                        <Tooltip title='Delete Terms & Conditions.' placement='bottom'>
                            <Button
    ...
    return (
        <>
            <Card sx={{ p: 5 }}>
                <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
                    ...
                    <Box display="flex" alignItems="center" gap={2}>
                        <GoBack label={'Terms & Conditions'} isBack={false} />
                    </Box>
                    ...
                    <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
                        ...
                        {/* {checkPermission('add_terms_and_conditions') && ( */}
                            <Grid item xs={12} sm="auto">


                                <Button onClick={() => setOpenAdd(true)} variant='contained'>
                                    Add Terms & Conditions <AddCircleOutlineIcon sx={{ ml: 1 }} />
                                </Button>
                            </Grid>
                        {/* )} */}
                    </Box>
            ...
            {openAdd && <AddTermsConditions open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
            {openDelete && (
                <DeleteDialogPopup show={openDelete} handleclose={() => setOpenDelete(false)} selectedItems={selectedItem.id}
                    fetchData={fetchGame}
                    label={'Are you sure! You want to delete.'} apiUrl={'api/v1/admin/deleteFaq?type=terms_and_conditions&id='} />
            )}
            {openEdit && (
                <AddTermsConditions open={openEdit} handleClose={() => setOpenEdit(false)}
                    fetchData={fetchGame}
