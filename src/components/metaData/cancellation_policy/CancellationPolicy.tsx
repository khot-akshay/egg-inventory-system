...
const CancellationPolicy = () => {
    ...
    const fetchGame = async () => {
        setLoading(true)
        let url = `/api/v1/admin/getAllFaqs?type=cancellation_policy&pageNo=${page}&limit=${pageSize}`
    ...
    const columns: GridColDef[] = [
...

        {
            ...
            renderCell: (params: GridCellParams) => (
                <>
                    {/* {checkPermission('update_cancellation_policy') && ( */}

                        <Tooltip title='Update Cancellation Policy.' placement='bottom'>
                    ...
                    {/* {checkPermission('delete_cancellation_policy') && ( */}

                        <Tooltip title='Delete Cancellation Policy.' placement='bottom'>
                            <Button
    ...
    return (
        <>
            <Card sx={{ p: 5 }}>
                <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
                    ...
                    <Box display="flex" alignItems="center" gap={2}>
                        <GoBack label={'Cancellation Policy'} isBack={false} />
                    </Box>
                    ...
                    <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
                        ...
                        {/* {checkPermission('add_cancellation_policy') && ( */}
                            <Grid item xs={12} sm="auto">
                                <Button onClick={() => setOpenAdd(true)} variant='contained'>
                                    Add Cancellation Policy <AddCircleOutlineIcon sx={{ ml: 1 }} />
                                </Button>
                            </Grid>
                        {/* )} */}
                    </Box>
            ...
            {openAdd && <AddCancellationPolicy open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
            {openDelete && (
                <DeleteDialogPopup show={openDelete} handleclose={() => setOpenDelete(false)} selectedItems={selectedItem.id}
                    fetchData={fetchGame}
                    label={'Are you sure! You want to delete.'} apiUrl={'api/v1/admin/deleteFaq?type=cancellation_policy&id='} />
            )}
            {openEdit && (
                <AddCancellationPolicy open={openEdit} handleClose={() => setOpenEdit(false)}
                    fetchData={fetchGame}
