...
const schema = yup.object().shape({
    ...
    description: yup
        ...
        .matches(/^\S(.*\S)?$/, 'Description cannot have leading or trailing spaces.')
        .matches(/^(?!.*\s{2,}).*$/, 'Description cannot have excessive spaces between words.')
...
const AddRefundPolicy = ({ open, handleClose, fetchData, selectedItem }: Props) => {
        ...
        try {
            ...
            toast.error(
                selectedItem
                    ...
                    : e?.response?.data?.message ?? 'Failed to add Refund Policy . Please try again.'
            )
        } finally {
            setIsLoading(false)
        }
        ...
        >
            ...
            >
                ...
                <Typography sx={{ fontSize: '25px', fontWeight: 'bold', textAlign: 'Start', flexGrow: 1, paddingLeft: '10px' }}>
                    {selectedItem ? 'Update' : 'Add'} Refund Policy {' '}
                </Typography>
