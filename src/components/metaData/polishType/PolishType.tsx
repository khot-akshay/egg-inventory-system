...
import AddCategories from './AddpolishType';
import AddProductsGrade from './AddpolishType';
import toast from 'react-hot-toast';
import AddPolishType from './AddpolishType';
...
const PolishType = () => {
  ...
  const fetchGame = async () => {
    setLoading(true);
    try {
      let url = `/api/v1/admin/polishTypes/getAllPolishTypes?pageNo=${page}&limit=${pageSize}`;
  ...
  const handleSwitchChange = async (event: React.ChangeEvent<HTMLInputElement>, params: any) => {
    ...
    try {
        await axiosInstance.post(`/api/v1/admin/polishTypes/updatePolishTypes/${params.id}`, { is_active: checked ? 1 : 0 })
        fetchGame()
  ...
  return (
    <>
      ...
      <Card sx={{ p: 5 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
          ...
          <Box display="flex" alignItems="center" gap={2}>
            <GoBack label="Polish Type" isBack={false} />
          </Box>
            ...
            <Grid item xs={12} sm="auto">
              ...
              {/* {checkPermission('add_brand') && ( */}
                <Button onClick={() => setOpenAdd(true)} variant='contained'>
                  Add Polish Type <AddCircleOutlineIcon sx={{ ml: 1 }} />
                </Button>
      ...
      {openAdd && <AddPolishType open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
      {openDelete && (
        <DeleteDialogPopup show={openDelete} handleclose={() => setOpenDelete(false)} selectedItems={selectedItem?.id}
          fetchData={fetchGame}
          label={'Are you sure! You want to delete.'} apiUrl={'api/v1/admin/polishTypes/deletePolishTypes/'} />
      )}
      {openEdit && (
        <AddPolishType open={openEdit} handleClose={() => setOpenEdit(false)}
          fetchData={fetchGame}
