...
const schema = yup.object().shape({
  name: yup
    ...
    // .matches(/^[a-zA-Z0-9\s]+$/, 'Polish Type cannot contain special characters.')
    .matches(/^\S(.*\S)?$/, 'Polish Type cannot have leading or trailing spaces.')
    .matches(/^(?!.*\s{2,}).*$/, 'Polish Type cannot have excessive spaces between words.')
    // .min(3, 'Polish Type must be at least 3 characters long.')
    .max(50, 'Polish Type cannot be more than 50 characters long.')
    .trim(),
  // display_order: yup
  //   .number()
...
const AddPolishType = ({ open, handleClose, fetchData, selectedItem }: Props) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      let url = ''
      if (selectedItem) {
        url = `/api/v1/admin/polishTypes/updatePolishTypes/${selectedItem.id}`
      } else {
        url = '/api/v1/admin/polishTypes/createPolishTypes'
      }
      ...
      // toast.error(
      //   selectedItem
      //     ? e?.response?.data?.message ?? 'Failed to update product grade. Please try again.'
      //     : e?.response?.data?.message ?? 'Failed to add product grade. Please try again.'
      // )
    } finally {
      setIsLoading(false)
    }
    ...
    >
      ...
      >
        ...
        <Typography sx={{ fontSize: '25px', fontWeight: 'bold', textAlign: 'Start', flexGrow: 1, paddingLeft: '10px' }}>
          {selectedItem ? 'Update' : 'Add'} Polish Type{' '}
        </Typography>
        ...
        <DialogContent dividers>
          <Grid container spacing={2}>
            <Grid item xs={12} md={12}>
              <RHFInput control={control} name={'name'} label={'Polish Type '} placeholder={'Polish Type'} mandatory />
            </Grid>
