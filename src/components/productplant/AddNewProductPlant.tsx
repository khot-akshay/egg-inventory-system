...
const AddNewProductPlant = ({ open, handleClose, fetchData, selectedItem,plantId }: Props) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (selectedItem) {
        ...
        url = '/api/v1/admin/plantProducts/createProductWithPlantProduct'
      }
    ...
    >
      ...
      >
        ...
        <Typography sx={{ fontSize: '25px', fontWeight: 'bold', textAlign: 'Start', flexGrow: 1, paddingLeft: '10px' }}>
          {selectedItem ? 'Update' : 'Add'} New Product{' '}
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <RHFAutoComplete
                ...
                valueKey="id"
                required
                extraParams={{ is_active: 1,plant_id: plantId }}
              />
            </Grid>
            <Grid item xs={12} md={6}>
...
export default AddNewProductPlant
