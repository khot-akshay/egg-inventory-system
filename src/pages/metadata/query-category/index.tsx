...
const QueryCategory = () => {
  ...
  const fetchGame = async () => {
    setLoading(true)
    let url = `/api/v1/admin/query/getAllQueryCategories?pageNo=${page}&limit=${pageSize}`;
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'name',
      headerName: 'Query Category Name',
      flex: 1,
    ...
    },
    {
      ...
      renderCell: (params: GridCellParams) => {
        return <div style={{ whiteSpace: 'normal', wordBreak: 'break-word', lineHeight: 1.5 }}>
          <p>{params.row?.user_type ? capitalizeFirstLetter(params.row.user_type) : 'NA'}</p>
    ...
    },
    {
      ...
      renderCell: (params: GridCellParams) => (
        <>
          {/* {checkPermission('update_query_category') && ( */}

            <Tooltip title='Update Query Category.' placement='bottom'>
          ...
          {/* {checkPermission('delete_query_category') && ( */}

            <Tooltip title='Delete Query Category.' placement='bottom'>
              <Button
  ...
  return (
    <>
      <Card sx={{ p: 5 }} className="bg-gray-50">
        <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" sx={{ mb: 3 }}>
          ...
          <Box display="flex" alignItems="center" gap={2}>
            <GoBack label={' Query Category'} isBack={false} />
          </Box>
          ...
          <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
            ...
            {/* {checkPermission('add_query_category') && ( */}
              <Grid item xs={12} sm="auto">
                <Button onClick={() => setOpenAdd(true)} variant='contained'>
                  Add Query Category <AddCircleOutlineIcon sx={{ ml: 1 }} />
                </Button>
              </Grid>
            {/* )} */}
          </Box>
      ...
      {openAdd && <Raisequery open={openAdd} handleClose={() => setOpenAdd(false)} fetchData={fetchGame} />}
      {openDelete && (
        <DeleteDialogPopup show={openDelete} handleclose={() => setOpenDelete(false)} selectedItems={selectedItem.id}
          fetchData={fetchGame}
          label={'Are you sure! You want to delete.'} apiUrl={'api/v1/admin/query/deleteQueryCategory/'} />
      )}
      {openEdit && (
        <Raisequery open={openEdit} handleClose={() => setOpenEdit(false)}
          fetchData={fetchGame}
